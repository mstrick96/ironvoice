...
|Step|Name                                                   |Status       |
|----|-------------------------------------------------------|-------------|
|1   |Skeleton + storage architecture                        |🟡 In progress|
...
1. **First launch** creates all four storage keys (`iv.plan.v2`, `iv.session.v2` (null), `iv.history.v2` (empty array), `iv.settings.v2`) with correct schema version and default values.
1. **Subsequent launches** read the stored values correctly and do not overwrite them with defaults.
1. **Corrupted JSON in any key** is detected, stashed to `iv.corrupt.v2.<timestamp>`, and the key is reset to defaults. A banner is shown to the user.
1. **Schema migration ladder** exists (even if trivially empty at v2 → v2) and is exercised on load.
1. **Close-tab rule** works: manually setting `lastActivityTimestamp` to a value more than 6 hours ago triggers auto-finalize on load (session becomes a history entry with `autoFinalizedOnClose: true`, session is cleared, welcome screen is shown).
1. **Storage inspector** screen exists and shows: size in KB, session count, date range, last export, last backup. Buttons present but may be stubs. Clear button requires typed confirmation.
1. **State machine skeleton** exists with all five states defined and a single `transition()` function that is the only way to change state. State transitions are logged to the diagnostic buffer.
1. **PWA detection** works: launching from home screen shows a block screen.
1. **HTTPS detection** works: loading from a non-HTTPS origin shows a clear error.
1. **Online/offline detection** works: `navigator.onLine` false shows a banner.
...
1. **Second launch:** close tab, reopen. Same state preserved.
1. **Corruption handling:** in dev tools, manually set `iv.plan.v2` to `not valid json`. Reload. Banner should appear; key should be reset to default; corrupted value should exist under `iv.corrupt.v2.<timestamp>`.
1. **Close-tab rule:** in dev tools, manually set `iv.session.v2` to a realistic session object with `lastActivityTimestamp` set to 7 hours ago. Reload. Session should be auto-finalized into history; welcome screen should show; banner mentioning “prior session auto-saved” should appear.
1. **PWA block:** Add to Home Screen. Launch from home screen icon. Block screen appears instead of welcome.
1. **HTTPS check:** temporarily download the HTML file and open it from Files app (file:// URL). HTTPS error should appear.
1. **Offline:** enable airplane mode. Reload. Offline banner should appear.
1. **Storage inspector:** navigate to Storage screen from welcome. All metrics display correctly.
...
1. `build_log.md` (this file, in its current state)
1. If we’re past Step 1: `IronVoice.html` (the current code)
